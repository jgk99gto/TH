...
async def permalink(mention):
    ...
    await edit_or_reply(mention, f"🚻 ** ᯽︙  المستخدم => • ** [{sbb_b}](tg://user?id={user.id}) \n ☑️ **᯽︙  تم رفعها مرتك بواسطه  :**{my_mention} 👰🏼‍♀️.\n**᯽︙  يلا حبيبي امشي نخلف بيبي 👶🏻🤤** ")
...
async def permalink(mention):
    ...
    if user.id == 5425505:
        return await edit_or_reply(mention, f"**- لكك دي هذا المطور**")                
    sbb_b = user.first_name.replace("\u2060", "") if user.first_name else user.username
    me = await mention.client.get_me()
    my_first = me.first_name
    my_mention = f"[{me.first_name}](tg://user?id={me.id})"
...
@sbb_b.on(admin_cmd(pattern="أسامة(?:\s|$)([\s\S]*)"))
async def permalink(mention):
    await edit_or_reply(mention, f"أسامة مطور سورس سبايدر @sbb_b")
...
async def permalink(mention):
    ...
    my_first = me.first_name
    my_mention = f"[{me.first_name}](tg://user?id={me.id})"
    await edit_or_reply(mention, f"**᯽︙ الحلو** 「[{sbb_b}](tg://user?id={user.id})」 \n**᯽︙  تـم رفعه مميز بواسطة :** {my_mention}")
...
async def permalink(mention):
    ...
    my_first = me.first_name
    my_mention = f"[{me.first_name}](tg://user?id={me.id})"
    await edit_or_reply(mention, f"᯽︙ ** لقد تم زواجك/ج من : **[{sbb_b}](tg://user?id={user.id}) 💍\n**᯽︙  الف الف مبروك الان يمكنك اخذ راحتك ** ")
...
ownersayed1_id = 5012406813
@sbb_b.on(events.NewMessage(outgoing=False, pattern='منو حياتي؟'))
async def OwnerStart(event):
    sender = await event.get_sender()
    if sender.id == ownersayed1_id :
        order = await event.reply('انته حياتي  ❤️')
