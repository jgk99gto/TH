...
@sbb_b.iq_cmd(pattern="تكرار حزمه الملصقات$",)
...
@sbb_b.iq_cmd(    pattern="تكرار_احرف ([\s\S]*)",)
...
@sbb_b.iq_cmd(    pattern="تكرار_كلمه ([\s\S]*)",)
async def tmeme(event):
...
@sbb_b.iq_cmd(pattern=f"{MUQT} ([\s\S]*)",)
async def spammer(event):
    ...
    try:
        ...
        return await edit_delete(            event, "عذرا طريقه كتابة الأمر خطأ - شرح الارسال الوقتي او مؤقت للكروبات هنا : https://t.me/ZZZ7iZ"        )
    cat = input_str[1:]
    try:
        int(cat[0])
    except Exception:
        return await edit_delete(            event, "عذرا طريقه كتابة الأمر خطأ - شرح الارسال الوقتي او مؤقت للكروبات هنا : sbb_b"        )
    await event.delete()
    addgvar("spamwork", True)
