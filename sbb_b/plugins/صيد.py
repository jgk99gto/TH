...
async def _(event):
    await event.edit(
        """
⩹⌯⊷━♢ ⦓ 𝗦𝗢𝗨𝗥𝗖𝗘 𝗖𝗔𝗟𝗜𝗣𝗛 ⦔ ♢━⊶⌯⩺
أوامر الصيد الخاصة بسورس الخليفه : 
    ...
    try:
        ch = await sbb_b(
            functions.channels.CreateChannelRequest(
                title="𝗦𝗢𝗨𝗥𝗖𝗘 𝗖𝗔𝗟𝗜𝗣𝗛 - صيد الخليفه ",
                about="This channel to hunt username by - @ZZZ7iZ ",
            )
        )
...
async def _(event):
    ...
    try:
        ...
        try:
            ch = await sbb_b(
                functions.channels.CreateChannelRequest(
                    title="SPIDER HUNTER - صيد الخليفه ",
                    about="This channel to hunt username by - @ZZZ7iZ ",
                )
            )
            ch = ch.updates[1].channel_id
